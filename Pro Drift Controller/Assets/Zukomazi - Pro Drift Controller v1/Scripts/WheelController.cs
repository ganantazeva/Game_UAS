    ...
    void wheelControl()
    {
        for (int i = 0; i < steerableWheels.Length; i++)
        {
          ...
          if (Input.GetKey(KeyCode.Space))
            {
                Debug.Log("Handbrake engaged!");
                RB.drag += handbrakeForce;
                foreach (WheelAlignment wheel in steerableWheels)
                {
                    wheel.wheelCol.brakeTorque = handbrakeForce;
                }
            }
            else
            {
                Debug.Log("Handbrake disengaged.");
                RB.drag =  0;
                foreach (WheelAlignment wheel in steerableWheels)
                {
